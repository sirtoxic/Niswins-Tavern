...
export async function generateCharacter() {
  ...
  try {
    if (!state.forgeRolledStats) rollForgeStats();
    const body = {
      concept,
      race,
      ...
      background_detail: state.selectedDetail,
...
export async function saveCharacter() {
  ...
  try {
    const r = await fetch('/api/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ character: state.currentCharacter, folder, history_id: state.currentHistoryId }),
    });
    const data = await r.json();
...
export function renderSheet(c, containerEl) {
...
export function renderCoreBox(label, value, sub) {
  const box = el('div', 'stat-box');
  box.innerHTML = `
...
export function renderACBreakdown(ac) {
...
export function renderSpellList(title, spells) {
  const wrap = el('div');
...
export function exportToFoundryJSON() {
  if (!state.currentCharacter) return;
...
export function _collectCharacterEdits() {
  const base = {
  ...
  const scoresChanged = _ABILITY_KEYS.some(a => newTotals[a] !== state.currentCharacter.ability_scores[a].total);
  if (scoresChanged) {
    const c = _recalcAbilityScores(state.currentCharacter, newTotals);
    return {
      ...base,
